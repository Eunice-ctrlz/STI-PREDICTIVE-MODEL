...
export const clinicianApi = {
  ...
  getProfile: () =>
    ...
    }>('/api/v1/clinicians/profile'),

  getAlerts: (status?: string) =>
    api.get<AlertSummary>(
      `/api/v1/clinicians/alerts${status ? `?status=${status}` : ''}`
    ),
  
  processAlert: (alertId: string, action: string, notes?: string) =>
    api.post<Alert>(`/api/v1/clinicians/alerts/${alertId}/action`, {
      action,
  ...
  getPopulationSummary: (county?: string) =>
    api.get<PopulationSummary>(
      `/api/v1/clinicians/population/summary${county ? `?county=${county}` : ''}`
    ),
  ...
  getDifferential: (
    ...
    }>('/api/v1/clinicians/differential', {
      symptoms,
      demographics,
