...
export const patientApi = {
  ...
  getSymptomQuestions: () =>
    api.get<Array<{
      ...
      category: string;
      help_text?: string;
    }>>('/patients/symptoms/questions'),

  submitAssessment: (data: AssessmentRequest) =>
    api.post<AssessmentResult>('/patients/assess', data),

  findClinics: (county: string, sub_county?: string, max_distance_km = 50) =>
    api.post<Array<{
      facility_id: string;
      name: string;
      county: string;
